...
這是一個貼近日常生活的 Flask 期末專題。使用者打開網站後，可以直接使用目前裝置的鏡頭：電腦使用前鏡頭，手機可切換前後鏡頭，把寶特瓶、手搖飲杯、外送餐盒、鋁箔包、紙箱或包裝標籤對準鏡頭。系統會先用瀏覽器端圖片模型推測物品，再搭配照片中的顏色、材質感、反光與邊緣特徵、OCR 文字、回收規則資料庫、公開資料查詢與網路搜尋摘要，推測分類與處理方式。

目前版本已完成可落地的網站、資料庫、相機頁面、圖片自動分類、影像特徵分析、OCR 文字讀取、公開資料查詢、文字規則分析與資料分析。

## 專題動機

每天都有人站在垃圾桶前猶豫：這個杯子能不能回收？鋁箔包算紙類嗎？外送餐盒太油還能丟回收嗎？本專題希望把環保知識變成人人打開網頁就能使用的小工具，讓回收分類像拍照一樣簡單。
...
- 手機前後鏡頭切換
- 拍照截圖輔助觀察，截圖只留在瀏覽器端
- 瀏覽器端 MobileNet 圖片自動分類，不需手動輸入材質
...
本機沒有設定 `DATABASE_URL` 時，系統會使用 `recycling_assistant.db` SQLite 檔案，方便開發與展示。

## 使用 Docker

```powershell
docker build -t recycling-assistant-flask .
docker run --rm -p 5000:5000 recycling-assistant-flask
```
...
- 圖片自動分類：使用瀏覽器端 MobileNet 先猜測照片中的物品，例如 bottle、carton、mask、mug、shoe，再映射到垃圾分類規則。
- 影像特徵分析：目前先從照片中央區域抓取顏色與材質線索，例如棕色紙板、白色餐盒、白色皺褶紙、金屬灰階反光、透明塑膠感、綠色玻璃感與邊緣密度，再把特徵轉成「紙箱、瓦楞紙、衛生紙、紙巾、鐵鋁罐、紙餐盒、保麗龍、寶特瓶、玻璃瓶」等查詢詞。
- 圖像分類模型：未來可改用 TensorFlow Lite、YOLO 或雲端視覺模型，進一步提升無文字物品的辨識穩定度。
